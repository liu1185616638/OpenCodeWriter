...
const APPLY_SESSION_TTL: Duration = Duration::from_secs(300);
...
pub fn split_thinking_tags(delta: &str, inside_thinking: &mut bool) -> Vec<(String, String)> {
    ...
    if *inside_thinking {
        let mut found_close: Option<(usize, usize, &str)> = None;
...
fn finish_cancelled(app: &AppHandle, session_id: &str, log_id: i64, output_chars: usize) -> String {
    finish_generation_log(app, log_id, "cancelled", "用户取消", output_chars);
    events::emit_cancelled(app, session_id, "用户取消了生成任务");
    CANCELLED_ERROR.to_string()
}

fn schedule_apply_session_cleanup(app: &AppHandle, session_id: &str) {
    let app = app.clone();
    let session_id = session_id.to_string();
    tauri::async_runtime::spawn(async move {
        sleep(APPLY_SESSION_TTL).await;
...
impl AiTaskService {
    ...
    /// Execute a task whose caller still needs to parse/apply the result.
    /// The session stays registered until the caller emits a terminal event.
    ...
    async fn execute_inner(
        ...
        let mut cancel_receiver = registry.register(session_id);

        let mut stream = tokio::select! {
            biased;
            _ = wait_for_cancel(&mut cancel_receiver) => {
                return Err(finish_cancelled(app, session_id, log_id, 0));
            }
            result = timeout(GENERATION_TIMEOUT, runtime.run(request)) => {
                match result {
                    Ok(Ok(stream)) => stream,
                    Ok(Err(error)) => {
        ...
        loop {
            let next_result = tokio::select! {
                biased;
                _ = wait_for_cancel(&mut cancel_receiver) => {
                    return Err(finish_cancelled(app, session_id, log_id, full_content.len()));
                }
                result = timeout(GENERATION_TIMEOUT, stream.next()) => {
                    match result {
                        Ok(Some(result)) => result,
                        Ok(None) => break,
                        Err(_) => {
                            let message = "生成超时（超过 5 分钟无响应），请检查模型配置或网络连接";
                            finish_generation_log(app, log_id, "timeout", message, full_content.len());
                            events::emit_error(app, session_id, message);
                            return Err(message.to_string());
                        }
                    }
            ...
            match next_result {
                Ok(delta) => match delta.delta_type {
                    AiDeltaType::Content => {
                        for (text, chunk_type) in split_thinking_tags(&delta.text, &mut inside_thinking) {
                    ...
                    AiDeltaType::Error => {
                        finish_generation_log(app, log_id, "failed", &delta.text, full_content.len());
                        events::emit_error(app, session_id, &delta.text);
                        return Err(delta.text);
                    }
                    AiDeltaType::Done => break,
                    AiDeltaType::ToolCall => {
                        events::emit_tool_call(app, session_id, &delta.text, &delta.payload);
                    }
                    AiDeltaType::ToolResult => {
                        let success = !delta.payload.is_null()
                            && delta.payload.get("success")
                                ...
                                .unwrap_or(true);
                        events::emit_tool_result(app, session_id, &delta.text, success, "", &delta.payload);
                    }
                    ...
                    AiDeltaType::SkillResult => {
                        ...
                        let error = delta.payload.get("error")
                            ...
                            .unwrap_or("");
                        events::emit_skill_result(app, session_id, &delta.text, success, error);
                    }
                    AiDeltaType::McpCall | AiDeltaType::McpResult => {
                        ...
                        match delta.delta_type {
                            AiDeltaType::McpCall => {
                                events::emit_mcp_call(app, session_id, &delta.text, &delta.payload);
                            }
                            AiDeltaType::McpResult => {
                                events::emit_mcp_result(app, session_id, &delta.text, success, error, &delta.payload);
                            }
                            _ => {}
                        }
                    }
                ...
                Err(error) => {
                    finish_generation_log(app, log_id, "failed", &error, full_content.len());
                    events::emit_error(app, session_id, &error);
                    return Err(error);
                }
            }
        ...
        if emit_done {
            ...
            schedule_apply_session_cleanup(app, session_id);
        }
        Ok(full_content)
    }
}
